#!/usr/bin/env python3
"""
BitsAndBytes 4-bit 양자화 스크립트
- Python 3.12 완벽 호환
- CPU/GPU 최대 활용
- Transformers 네이티브 통합
"""
...
def quantize_bnb(
    ...
    Args:
        ...
        load_in_4bit: 4-bit 양자화 (True) vs 8-bit (False)
        bnb_4bit_compute_dtype: 계산용 dtype (float16 추천)
        bnb_4bit_use_double_quant: 이중 양자화 (추가 압축, 품질 유지)
        bnb_4bit_quant_type: 양자화 타입 (nf4 or fp4)
            - nf4: NormalFloat4 (추천, 품질 최고)
            - fp4: Float4 (빠름)
    """
    ...
    print("\n" + "="*80)
    print(" BitsAndBytes 양자화 시작")
    print("="*80)
    print(f"시작 시간: {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"입력 모델: {model_path}")
    print(f"출력 경로: {output_path}")
    bits = "4-bit" if load_in_4bit else "8-bit"
    print(f"설정: {bits}, dtype={bnb_4bit_compute_dtype}, quant_type={bnb_4bit_quant_type}")
    print("="*80 + "\n")
    ...
    bnb_config = BitsAndBytesConfig(
        ...
        llm_int8_threshold=6.0,  # 8-bit 전용
        llm_int8_has_fp16_weight=False  # 8-bit 전용
    )
...
Assistant: """
    ...
    print("\n특징:")
    print("  ✓ GTX 1660 (6GB VRAM) 지원")
    print("  ✓ Python 3.12 완벽 호환")
    print("  ✓ Transformers 네이티브 통합")
    print("  ✓ 품질 손실 최소 (1-2%)")
...
def main():
    parser = argparse.ArgumentParser(
        description="BitsAndBytes 양자화 (Python 3.12 호환)",
...
# 8-bit 양자화 (더 높은 품질)
python bnb.py --bits 8

# FP4 타입 (더 빠름)
python bnb.py --quant-type fp4

# 커스텀 설정
python bnb.py \\
    --model /home/work/eeve-merged-checkpoint-500 \\
    --output /home/work/eeve-bnb-4bit \\
    --bits 4 \\
    --quant-type nf4 \\
    --double-quant
        """
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="/home/work/tesseract/quant/eeve-bnb-4bit",
        help="양자화 모델 저장 경로"
    )
